...
-----------------------------------------------------------
The ORIGINAL source can be wrong — the kernel AND its harness both. Keep the user's
original files PRISTINE; the loop works on copies/wrappers under artifacts/:

  * CALIBRATE (before the loop): with the user's approval, fix genuine correctness /
    measurement bugs first so the baseline measures something real. (Fixing a real
    bug is allowed; silently changing WHAT is measured is not.)
      - the original HARNESS is corrected then FROZEN; you drive it via this harness.py.
      - the original KERNEL is COPIED into artifacts/<problem>/ as the working
        EDIT_TARGET; any correctness fix is applied to that copy, not the original.
  * LOOP (autonomous): edit ONLY the working kernel copy (EDIT_TARGET under artifacts/).
    Never edit the original kernel or the original harness — the originals stay frozen
    and trustworthy. (If there was no starting kernel, the from-scratch initial version
    IS the working copy.)

So `HARNESS_REF` points at the corrected-and-frozen original harness (read / compile /
drive it, never edit), and `EDIT_TARGET` points at the working kernel COPY under
artifacts/ — never the user's original kernel file.
...
# Make the skill's bundled libs importable regardless of cwd.
...
# (Inside this template file itself the skill dir is ONE dir up: templates/..)
SKILL = os.environ.get(
    "OPTIMIZE_KERNELS_SKILL",
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..")),
)
sys.path.insert(0, SKILL)
from tuning import autotune  # noqa: E402  (autotune lives in the `tuning/` concern dir)

# --- problem-specific paths (TODO) -----------------------------------------
EDIT_TARGET = "artifacts/<problem>/<kernel-source>"          # WORKING COPY (copied from the original at calibrate); the ONLY file the loop edits
HARNESS_REF = "optimize-kernels/<problem>/<harness-source>"  # ORIGINAL harness: corrected + frozen; driven, never edited
BUILD_DIR = "artifacts/<problem>/build"
# Arch is DETECTED at calibrate (rocm_agent_enumerator / rocminfo) — do NOT hardcode it.
# When generating harness.py, bake in the detected value (gfx90a / gfx942 / gfx950).
# NOTE: HW counter availability is arch-dependent — validate the counter set per arch.
ARCH = os.environ.get("OPTIMIZE_KERNELS_ARCH", "gfx942")  # TODO: set to the calibrated arch
