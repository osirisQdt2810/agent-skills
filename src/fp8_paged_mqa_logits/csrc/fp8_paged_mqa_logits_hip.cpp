...
void launch_fp8_paged_mqa_logits(
    ...
    #define LAUNCH_V9(NW, CK)                                                              \
       hipLaunchKernelGGL(( v9::fp8_paged_mqa_logits_kernel<NW, CK>), dim3(grid), dim3(NW * WARP_SIZE), 0, stream,      \
            d_q, d_kv, d_w, d_ctx, d_bt, d_out,                                            \
            batch_size, next_n, max_blocks_per_seq, max_model_len, index_dim, SplitKV,    \
            block_size);
    ...
    #define LAUNCH_V10(NW, CK, KB)                                                         \
       hipLaunchKernelGGL(( v10::fp8_paged_mqa_logits_kernel<NW, CK, KB>), dim3(grid), dim3(NW * WARP_SIZE), 0, stream, \
            d_q, d_kv, d_w, d_ctx, d_bt, d_out,                                            \
            batch_size, next_n, max_blocks_per_seq, max_model_len, index_dim, SplitKV);
